...
In the upstream services YAML, [`sql_statement_permissions`](https://github.com/Snowflake-Labs/mcp#sql-execution) filters SQL categories. The [bundled example](../lenses/databases/snowflake-labs/services.example.yaml) disables common write categories and `Unknown`, but keeps `Command: True` for `SHOW`; upstream also places `CALL` and `GRANT` in that category. These filters alone do not make the standalone server read-only.

For read-only use, restrict [procedure `USAGE` grants](https://docs.snowflake.com/en/user-guide/security-access-control-privileges#stored-procedure-privileges) to an administrator-reviewed allowlist, including access inherited through other roles or [`PUBLIC`](https://docs.snowflake.com/en/user-guide/security-access-control-overview). [Owner's-rights procedures](https://docs.snowflake.com/en/developer-guide/stored-procedure/stored-procedures-rights) can write with their owner's privileges even when the caller has no table write privileges. Review their operations before granting access. JanuScope's `sqlGuard` separately refuses `CALL` and `GRANT` through `run_snowflake_query` while retaining `SHOW`; it does not inspect procedure bodies.
