...
2. **Advise**: `instructions` adds configured policy text to tool descriptions, including a SURFACE BOUNDARY paragraph when the lens supplies one. `contextInjection` adds operator-supplied inline or file content to selected tool descriptions. This guidance cannot enforce restrictions on the host's other tools (terminal, vendor CLI, sibling MCPs).
...
- **Request gates refuse on handler exceptions.** A failing `block` or `sqlGuard` handler withholds the unchecked message. Messages with an `id` produce a JSON-RPC error for the side awaiting a response: string, numeric and `null` IDs are retained, while other ID types become `null`. Messages without an `id`, including notifications, are dropped without an error response. See [`src/pipeline.ts`](./src/pipeline.ts).
